...
for (SSP_i in c(1:length(SSPs))){
  ...
  for (exposure in c(0, 5)){
    ...
    for (GCM_i in c(1:length(GCMs))){
      GCM<-GCMs[GCM_i]
      target_folder<-sprintf("../../Objects/density_based_pathway_10km/%s_%s_exposure_%d.rda", 
                             GCM, SSP, exposure)
...
p
ggsave(p, filename="../../Figures/density_based_pathway/density_based_pathway_10km.png", width=13, height=6)
...
p<-ggplot(df_final_alt, aes(x=alt, y=mean_count))+geom_point(size=1, color="grey")+
  ...
  theme_bw()
p
ggsave(p, filename="../../Figures/density_based_pathway/density_vs_elevation_10km.png", width=13, height=6)
p<-ggplot(df_final, aes(x=slope, y=mean_count))+geom_point(size=1, color="grey")+
  ...
  theme_bw()
p
ggsave(p, filename="../../Figures/density_based_pathway/density_vs_slope_10km.png", width=13, height=6)
...
for (SSP_i in c(1:length(SSPs))){
  ...
  for (exposure_l in c(0, 5)){
    ...
    ggsave(p_item, filename=
             sprintf("../../Figures/density_based_pathway/density_based_pathway_%s_exposure_%d_10km.png", SSP_l, exposure_l),
           width=12, height=6)
  }
}
if (F){
  ...
  saveRDS(df_all_by_year, "../../Figures/density_based_pathway/df_all_by_year_10km.rda")
}

df_all_by_year<-readRDS("../../Figures/density_based_pathway/df_all_by_year_10km.rda")
df_all_by_year<-df_all_by_year[mask_100km %in% mask_p$mask_100km]
...
for (yyy in c(2021:2100)){
  ...
  ggsave(p, filename = sprintf("../../Figures/density_based_pathway/Movies_10km/%d.png", yyy),
         width=13, height=4)
}

#cd /media/huijieqiao/Speciation_Extin/Sp_Richness_GCM/Figures/density_based_pathway/Movies_10km
#ffmpeg -r 5 -start_number 2021 -i %04d.png -y ../density_based_pathway_10km.mp4
