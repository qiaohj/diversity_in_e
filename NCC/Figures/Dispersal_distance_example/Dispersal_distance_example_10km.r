...
exposure<-c(0, 5)
tempelate_10km<-"../../Objects/Dispersal/Birds/%s/UKESM1_SSP585_%d_dispersal_1_10km.rda"
tempelate_100km<-"../../Objects/Dispersal/Birds/%s/UKESM1_SSP585_%d_dispersal_1.rda"
...
for (i in c(1:nrow(sub_bird_disp))){
  ...
  for (e in exposure){
    item<-readRDS(sprintf(tempelate_100km, gsub(" ", "_", sp), e))
    bird_disp_item<-bird_disp[iucn_name==sp]
    year=2022
...
range(all_item$disp)
p1<-ggplot(all_item)+
  geom_histogram(aes(x=disp, fill=factor(exposure)), bins=50, boundary = 0)+
  theme_bw()+
...
p
ggsave(p1, filename="../../Figures/Dispersal_distance_example/Dispersal_distance_example.png", width=10, height=3)
ggsave(p1, filename="../../Figures/Dispersal_distance_example/Dispersal_distance_example.pdf", width=10, height=3)
...
p1<-ggplot(all_item)+
  geom_histogram(aes(x=disp, fill=factor(exposure)), bins=50)+
  theme_bw()+
  facet_wrap(~label, nrow=1)+
  labs(x="Mean dispersal distance (km)", y="count", fill="Exposure")
