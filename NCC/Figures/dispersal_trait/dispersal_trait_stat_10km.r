...
p
ggsave(p, filename = "../../Figures/dispersal_traits/dispersal_stat_10km.png", width=3, height=4)
SSS<-"SSP119"
for (SSS in unique(df_se_across_GCM$SSP)){
  p<-ggplot(df_se_across_GCM%>%dplyr::filter(SSP==SSS))+
    ...
    theme(legend.position="top",
          ...
          axis.line=element_line())
  p
  ggsave(p, filename = sprintf("../../Figures/dispersal_traits/dispersal_stat_%s_10km.png", SSS), width=3, height=4)
}
