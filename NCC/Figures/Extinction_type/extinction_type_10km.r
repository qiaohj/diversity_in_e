...
if (F){
  ...
  saveRDS(result, "../../Objects/Extinction_type/Extinction_type_10km.rda")
}
df_sp_list<-list()
...
result<-readRDS("../../Objects/Extinction_type/Extinction_type_10km.rda")
...
write.table(result_count_se, "../../Objects/Extinction_type/Extinction_type_10km.csv", row.names = F, sep=",")
...
result_count_df<-data.table(result_count)
result_count_df[dispersal==1&type=="Unsuitable"&exposure==0]
result_count_se_df<-data.table(result_count_se)
result_count_se_df[dispersal=="with dispersal"&exposure==" no exposure"&type=="Unsuitable"]
