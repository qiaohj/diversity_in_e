...
for (i in c(1:nrow(coms))){
  ...
  target<-sprintf("../../Raster/VoCC/%s/%s/bio%d_voccMag.tif", sss_str[1], sss_str[2], item$bio)
  if (file.exists(target)){
    next()
  }
  folder<-sprintf("../../Raster/VoCC/%s/%s", sss_str[1], sss_str[2])
  ...
  writeRaster(v[[2]], sprintf("../../Raster/VoCC/%s/%s/bio%d_voccAng.tif", sss_str[1], sss_str[2], item$bio), overwrite=T)
}
