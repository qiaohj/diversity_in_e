  ...
  for (exposure in c(0, 5)){
    for (g in c("Birds", "Mammals")){
      ...
      for (dis in c(0, 1)){
        ...
        sp_richness<-readRDS(sprintf("../../Objects/Diversity_exposure_%d_dispersal_%d_10km_2_100km/%s/EC-Earth3-Veg_SSP119/indices_df.rda", 
                                     exposure, dis, g))
        ...
        where_extinct_item<-where_extinct_item%>%dplyr::filter(extinct_ratio<=1)
        gcm_str<-"UKESM1"
        ssp_str<-"SSP245"
        for (gcm_str in unique(where_extinct_item$GCM)){
          for (ssp_str in unique(where_extinct_item$SSP)){
            item<-where_extinct_item%>%dplyr::filter(SSP==ssp_str&GCM==gcm_str)
            p_mask<-data.frame(rasterToPoints(mask))
            colnames(p_mask)[3]<-"mask_100km"
            p_mask<-left_join(p_mask, item, by="mask_100km")
            r<-mask
            values(r)[!is.na(values(mask))]<-p_mask$n_sp
            
            #plot(r, main=paste(g, SSP_i, dis))
            
            writeRaster(r, sprintf("../../Figures/when_where_extinction_exposure_%d/where/TIFF/%s_%s_%s_extinct_n_dispersal_%d_10km.tif",
                                   exposure, g, gcm_str, ssp_str, dis), overwrite=T)
            
            r<-mask
            values(r)[!is.na(values(mask))]<-p_mask$extinct_ratio
            
            #plot(r, main=paste(g, SSP_i, dis))
            
            writeRaster(r, sprintf("../../Figures/when_where_extinction_exposure_%d/where/TIFF/%s_%s_%s_extinct_ratio_dispersal_%d_10km.tif",
                                   exposure, g, gcm_str, ssp_str, dis), overwrite=T)
      ...
      saveRDS(where_extinct_all, sprintf("../../Objects/when_where_extinction_exposure_%d/where_extinct_%s_10km.rda", 
                                        exposure, g))
    }
  }
}
...
for (exposure in c(0, 5)){
  for (g in c("Birds", "Mammals")){
    for (ttt in c(2)){
      where_extinct<-readRDS(sprintf("../../Objects/when_where_extinction_exposure_%d/where_extinct_%s_10km.rda",
                                     exposure, g))
      for (da in c(0:1)){
        print(paste(g, exposure, da))
        png(sprintf("../../Figures/when_where_extinction_exposure_%d/where/%s_extinct_ratio_dispersal_%d_10km.png", 
        ...
        for (SSP_i in c("SSP119", "SSP245", "SSP585")){
          ...
          writeRaster(r, sprintf("../../Figures/when_where_extinction_exposure_%d/where/%s_%s_extinct_ratio_dispersal_%d_10km.tif",
                                 exposure, g, SSP_i, da), overwrite=T)
        }
        dev.off()
        
        png(sprintf("../../Figures/when_where_extinction_exposure_%d/where/%s_n_extinct_dispersal_%d_10km.png", 
                    exposure, g, da), width=1000, height=200)
        par(mfrow=c(1, 3))
        for (SSP_i in c("SSP119", "SSP245", "SSP585")){
          item<-where_extinct%>%dplyr::filter((SSP==SSP_i)&(dispersal==da))
          ...
          writeRaster(r, sprintf("../../Figures/when_where_extinction_exposure_%d/where/%s_%s_n_extinct_dispersal_%d_10km.tif", 
                                 exposure, g, SSP_i, da), overwrite=T)
        }
        dev.off()
      }
...
if (F){
    ...
    for (SSP_i in c("SSP119", "SSP245", "SSP585")){
      for (da in c(0:1)){
        ...
        for (g in c("Birds", "Mammals")){
          ...
          ratio<-raster(sprintf("../../Figures/when_where_extinction_exposure_%d/where/%s_%s_extinct_ratio_dispersal_%d_10km.tif", 
                                exposure, g, SSP_i, da))
          ...
          n_ext<-raster(sprintf("../../Figures/when_where_extinction_exposure_%d/where/%s_%s_n_extinct_dispersal_%d_10km.tif", 
                                exposure, g, SSP_i, da))
          ...
          print(quantile(ratio$V))
          
        }
        print(quantile(ratio_all$V, na.rm=T))
  ...
  saveRDS(ratio_final, "../../Figures/when_where_extinction_all/ratio_final_10km.rda")
  saveRDS(n_ext_final, "../../Figures/when_where_extinction_all/n_ext_final_10km.rda")
  saveRDS(ratio_final_group, "../../Figures/when_where_extinction_all/ratio_final_group_10km.rda")
  saveRDS(n_ext_final_group, "../../Figures/when_where_extinction_all/n_ext_final_group_10km.rda")
...
for (da in c(0:1)){
  ...
  p
  ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/ratio_final_dispersal_%d_10km.png", da), 
         width=9, height=4)
  ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/ratio_final_dispersal_%d_10km.pdf", da),
         width=9, height=4)
  ...
  p
  ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/n_ext_final_dispersal_%d_10km.png", da),
         width=9, height=4)
  ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/n_ext_final_dispersal_%d_10km.pdf", da),
         width=9, height=4)
  ...
  for (g in c("Birds", "Mammals")){
    ...
    p
    ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/ratio_final_dispersal_%d_%s_10km.png", da, g), 
           width=9, height=4)
    ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/ratio_final_dispersal_%d_%s_10km.pdf", da, g),
           width=9, height=4)
    ...
    p
    ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/n_ext_final_dispersal_%d_%s_10km.png", da, g),
           width=9, height=4)
    ggsave(p, filename=sprintf("../../Figures/when_where_extinction_all/n_ext_final_dispersal_%d_%s_10km.pdf", da, g),
           width=9, height=4)
  }
