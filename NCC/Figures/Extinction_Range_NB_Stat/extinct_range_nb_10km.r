...
for (g in c("Birds", "Mammals")){
  ...
  for (exposure in c(0, 5)){
    extinct<-readRDS(sprintf("../../Objects/when_where_extinction_exposure_%d/%s_10km.rda", exposure, g))
    extinct<-rbindlist(extinct)
...
for (i in c(1:3)){
  ...
  ggsave(ppp, filename=sprintf("../../Figures/Extinction_Range_NB_Stat/%s_10km.png", var),
         width=15, height=4)
  ggsave(ppp, filename=sprintf("../../Figures/Extinction_Range_NB_Stat/%s_10km.pdf", var),
         width=15, height=4)
}
...
p1
saveRDS(p1, "../../Figures/NB_hist_combined/nb_temp_10km.rda")
ggsave(p1, filename=sprintf("../../Figures/N_Extinction/NB_Extinct/Extinction_hist_nb_temp_10km.pdf"), width=12, height=6)
ggsave(p1, filename=sprintf("../../Figures/N_Extinction/NB_Extinct/Extinction_hist_nb_temp_10km.png"), width=12, height=6)
...
p2
saveRDS(p2, "../../Figures/NB_hist_combined/nb_prec_10km.rda")
...
p3<-ggplot(df)+
  ...
  facet_grid(exposure~Label)
p3
saveRDS(p3, "../../Figures/NB_hist_combined/disp_dist_10km.rda")


p2<-p2+
  theme(strip.background.x = element_blank(),
        strip.text.x = element_blank())
ggsave(p2, filename=sprintf("../../Figures/N_Extinction/NB_Extinct/Extinction_hist_nb_prec_10km.pdf"), width=12, height=6)
ggsave(p2, filename=sprintf("../../Figures/N_Extinction/NB_Extinct/Extinction_hist_nb_prec_10km.png"), width=12, height=6)


pp<-ggarrange(p1, p2, ncol=1, nrow=2)
ggsave(pp, filename=sprintf("../../Figures/N_Extinction/NB_Extinct/Extinction_hist_nb_combined_10km.pdf"), width=12, height=6)
ggsave(pp, filename=sprintf("../../Figures/N_Extinction/NB_Extinct/Extinction_hist_nb_combined_10km.png"), width=12, height=6)
...
write.csv(df, "../../Figures/raw_result_10km.csv", row.names=F)
...
write.csv(all_result, "../../Figures/NB_hist_combined/p_values_with_scaled_max_disp_dist_10km.csv", row.names=F)

all_result_raw<-read.csv("../../Figures/NB_hist_combined/p_values_with_scaled_max_disp_dist_10km.csv")
...
all_result<-read.csv("../../Figures/NB_hist_combined/p_values_with_scaled_max_disp_dist_10km.csv")
