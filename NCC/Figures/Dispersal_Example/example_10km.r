...
print(item_str)
item<-readRDS(sprintf("../../Objects/stacked_layers_2021_2100_list_10km_%s.rda", item_str))
...
for (year_i in predict_range){
  ...
  if (dispersal==0){
    ...
    if (T){
      if (with_climate_change){
        png(sprintf("../../Figures/Example/dispersal_with_climate_10km/%d.png", year_i))
      }else{
        png(sprintf("../../Figures/Example/dispersal_no_climate_10km/%d.png", year_i))
      }
      mask_item<-crop(mask_10km, extent(1000000,   5000000,  -3000000, 3500000))
      
      plot(mask_item, col="lightgrey", main=year_i, legend=FALSE)
      if (with_climate_change){
        suitable_area<-mask_10km
        values(suitable_area)<-NA
        values(suitable_area)[item[year==year_i]$mask_10km]<-1
        suitable_area<-crop(suitable_area, extent(1000000,   5000000,  -3000000, 3500000))
        plot(suitable_area, col=scales::alpha("indianred1", 0.2), add=T, legend=FALSE)
      }
...
if (F){
  ...
  for (year in c(2021:2100)){
    print(year)
    png(sprintf("../../Figures/Example/dispersal_movies_10km/%d.png", year), 
    ...
    path1<-image_read(sprintf("../../Figures/Example/dispersal_no_climate_10km/%d.png", year))
    path2<-image_read(sprintf("../../Figures/Example/dispersal_with_climate_10km/%d.png", year))
    plot(path1)
    plot(path2)
    dev.off()
  }
...
#cd /media/huijieqiao/Speciation_Extin/Sp_Richness_GCM/Figures/Example/dispersal_movies_10km
#ffmpeg -r 5 -start_number 2021 -i %04d.png -y ../dispersal_movies_10km.mp4