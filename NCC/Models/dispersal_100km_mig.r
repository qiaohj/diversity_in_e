...
if (F){
  ...
  for (n in names(future_env_layers)){
    print(n)
    saveRDS(future_env_layers[[n]], sprintf("../../Objects/stacked_layers_2021_2100_list_100km_%s.rda", n))
  }
}
...
group_full_sum_area<-group_full_sum_area[(SCINAME  %in% group_full[estimated_disp>dispersal_threshold]$SCINAME)|
                                           (SCINAME %in% distribution_all$sp)]
...
for (i in 1:length(group_full_sum_area$SCINAME)) {
  ...
  for (j in 1:nrow(coms)){
    ...
    for (item_str in esm_ssp){
      ...
      saveRDS(NULL, target)
      check_point<- sprintf("%s/initial_disp_100km_exposure_%d_dispersal_%d_seasonal_2.rda", 
