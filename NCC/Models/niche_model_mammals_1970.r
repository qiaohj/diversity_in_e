...
mammals_full<-readRDS("../../Objects/IUCN_List/Mammals_df.rda")

unique <- unique(mammals_full$SP)
...
start_env_layers<-start_env_layers_se[year>=1970]
...
mammal_full_sum_area<-mammals_full
...
for (i in 1:length(mammal_full_sum_area$SP)) {
  
  bi<-mammal_full_sum_area$SP[i]
  #bi="Pseudophryne occidentalis"
  print(paste(i, length(unique), bi))
  target_folder<-sprintf("../../Objects/Dispersal/Mammals/%s/fit_1970.rda", gsub(" ", "_", bi))
  ...
  nb_v<-start_env_layers[mask_10km %in% nb_p$mask_10km]
