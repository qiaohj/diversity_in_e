...
for (i in 4400:length(bird_full_sum_area$SCINAME)) {
  
  bi<-bird_full_sum_area$SCINAME[i]
  #bi="Pseudophryne occidentalis"
  print(paste(i, length(unique), bi))
  target<-sprintf("../../Objects/Dispersal/Birds/%s/fit_seasonal_%d.rda", gsub(" ", "_", bi), SEASONAL)
  ...
  tmp_sf<-tmp_sf[which(tmp_sf$SEASONAL==SEASONAL),]
  if (nrow(tmp_sf)==0){
    next()
  }
