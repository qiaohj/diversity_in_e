...
bird_full<-readRDS("../../Objects/IUCN_List/Birds_df.rda")

unique <- unique(bird_full$SP)
...
for (i in 1:length(bird_full_sum_area$SP)) {
  
  bi<-bird_full_sum_area$SP[i]
  #bi="Pseudophryne occidentalis"
  print(paste(i, length(unique), bi))
  target_folder<-sprintf("../../Objects/Dispersal/Birds/%s/fit_1970.rda", gsub(" ", "_", bi))
