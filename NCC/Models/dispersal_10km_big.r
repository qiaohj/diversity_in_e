...
distribution_threshold2<-200
dispersal_threshold2<-100
#dispersal_threshold<-890
#distribution_threshold<-2000

distribution_all<-readRDS("../../Objects/N_cell_init_distribution.rda")
distribution_all<-distribution_all[between(N, distribution_threshold, distribution_threshold2)]
group_full_sum_area<-group_full_sum_area[SCINAME  %in% 
                                           group_full[between(estimated_disp, 
                                                              dispersal_threshold,
                                                              dispersal_threshold2)]$SCINAME]
...
for (i in 1:length(group_full_sum_area$SCINAME)) {
  ...
  for (j in 1:nrow(coms)){
    ...
    print(paste("20220215", i, length(group_full_sum_area$SCINAME), bi, 
    ...
    for (item_str in esm_ssp[esm_i]){
      ...
      for (year_i in predict_range){
        if (dispersal==1){
          ...
          print(paste("20220215 (start time", start_time, ", current time", 
                      current_time,
