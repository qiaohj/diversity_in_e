...
unique <- unique(sp_df@data$SCINAME)
...
for (i in 1:length(unique)) {
  ...
  target<-sprintf("../Object/IUCN_Distribution/Birds/%s.rda", gsub(" ", "_", bi))
  ...
  if (n_pixel>0){
    ...
    print(target)
    saveRDS(ppp, target)
    writeRaster(rp, sprintf("../Raster/IUCN_Distribution/Birds/%s.tif", gsub(" ", "_", bi)), overwrite=T)
  }
  if (n_pixel>3){
