...
if (F){
  ...
  for (g in c("Amphibians", "Birds", "Reptiles", "Mammals")){
    ...
    df$mask_index<-extract(mask, df[, c("x", "y")])
    ...
    sp_richness<-readRDS(sprintf("../../Objects/Diversity/%s/EC-Earth3-Veg_SSP119_1_1/indices_df.rda", group))
    
    sp_richness<-sp_richness[["2014"]][["species.richness"]]
    ...
    saveRDS(where_extinct_se, sprintf("../../Objects/when_where_extinction/where_extinct_%s.rda", g))
  }
}
g<-"Amphibians"
mask<-raster("../../Raster/mask_index.tif")
...
for (g in c("Amphibians", "Birds", "Mammals", "Reptiles")){
  print(g)
  where_extinct<-readRDS(sprintf("../../Objects/when_where_extinction/where_extinct_%s.rda", g))
  png(sprintf("../../Figures/when_where_extinction/where/%s.png", g), width=1000, height=200)
  par(mfrow=c(1, 3))
  for (SSP_i in c("SSP119", "SSP245", "SSP585")){
    ...
    writeRaster(r, sprintf("../../Figures/when_where_extinction/where/%s_%s.tif", g, SSP_i), overwrite=T)
  }
  dev.off()
  
  png(sprintf("../../Figures/when_where_extinction/where/%s_n_extinct.png", g), width=1000, height=200)
  par(mfrow=c(1, 3))
  for (SSP_i in c("SSP119", "SSP245", "SSP585")){
    item<-where_extinct%>%dplyr::filter(SSP==SSP_i)
    p_mask<-data.frame(rasterToPoints(mask))
    ...
    writeRaster(r, sprintf("../../Figures/when_where_extinction/where/%s_%s_n_extinct.tif", g, SSP_i), overwrite=T)
  }
  dev.off()
}
