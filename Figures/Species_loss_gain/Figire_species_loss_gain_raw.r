...
for (SSP_i in SSPs){
  for (k in c(1:length(dispersals))){
    ...
    for (year_str in names(df_end_full_list)){
      ...
      df_end_se$p_2020_cut<-cut(df_end_se$mean_n_2020, breaks=cuts_x, labels=cuts_x[2:length(cuts_x)], include.lowest=T)
      df_end_se$p_year_cut<-cut(df_end_se$mean_n_year, breaks=cuts_y, labels=cuts_y[2:length(cuts_y)], include.lowest=T)
