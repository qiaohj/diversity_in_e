...
if (F){
  ...
  for (threshold in c(1, 5)){
    for (g in c("Amphibians", "Birds", "Reptiles", "Mammals")){
      ...
      for (ttt in c(0, 1, 2)){
        df<-df_bak%>%dplyr::filter(N_CELL>ttt)
        ...
        sp_richness<-readRDS(sprintf("../../Objects_Full_species/Diversity_%d/%s/EC-Earth3-Veg_SSP119_1/indices_df.rda", 
                                     threshold, g))
        ...
        if (F){
          ...
          xx<-readRDS("../../Objects_Full_species/IUCN_Distribution/Amphibians/Rana_amurensis.rda")
          xx$index<-extract(mask, xx[, c("x", "y")])
          xx%>%dplyr::filter(index==1335)
          
          xx<-readRDS("../../Objects_Full_species/IUCN_Distribution/Amphibians/Salamandrella_keyserlingii.rda")
          xx$index<-extract(mask, xx[, c("x", "y")])
          xx%>%dplyr::filter(index==1335)
        }
        ...
        saveRDS(where_extinct_se, sprintf("../../Objects_Full_species/when_where_extinction_%d/where_extinct_%s_ttt_%d.rda", 
                                          threshold, g, ttt))
      }
    }
  }
  ...
  for (g in c("Amphibians", "Birds", "Mammals", "Reptiles")){
    for (ttt in c(2)){
      ...
      for (da in c(0:1)){
        ...
        png(sprintf("../../Figures_Full_species/when_where_extinction_%d/where/%s_extinct_ratio_dispersal_%d_ttt_%d.png", 
                    threshold, g, da, ttt), width=1000, height=200)
        par(mfrow=c(1, 3))
        for (SSP_i in c("SSP119", "SSP245", "SSP585")){
          ...
          writeRaster(r, sprintf("../../Figures_Full_species/when_where_extinction_%d/where/%s_%s_extinct_ratio_dispersal_%d_ttt_%d.tif",
                                 threshold, g, SSP_i, da, ttt), overwrite=T)
        }
        dev.off()
        
        png(sprintf("../../Figures_Full_species/when_where_extinction_%d/where/%s_n_extinct_dispersal_%d_ttt_%d.png", 
                    threshold, g, da, ttt), width=1000, height=200)
        par(mfrow=c(1, 3))
        for (SSP_i in c("SSP119", "SSP245", "SSP585")){
          item<-where_extinct%>%dplyr::filter((SSP==SSP_i)&(dispersal==da))
          ...
          writeRaster(r, sprintf("../../Figures_Full_species/when_where_extinction_%d/where/%s_%s_n_extinct_dispersal_%d_ttt_%d.tif", 
                                 threshold, g, SSP_i, da, ttt), overwrite=T)
        }
        dev.off()
      }
...
if (F){
      ...
      for (SSP_i in c("SSP119", "SSP245", "SSP585")){
        for (da in c(0:1)){
          ...
          for (g in c("Amphibians", "Birds", "Mammals", "Reptiles")){
            ...
            ratio<-raster(sprintf("../../Figures_Full_species/when_where_extinction_%d/where/%s_%s_extinct_ratio_dispersal_%d_ttt_%d.tif", 
            ...
            n_ext<-raster(sprintf("../../Figures_Full_species/when_where_extinction_%d/where/%s_%s_n_extinct_dispersal_%d_ttt_%d.tif", 
                                  threshold, g, SSP_i, da, ttt))
  ...
  saveRDS(ratio_final, "../../Figures_Full_species/when_where_extinction_all/ratio_final_with_ttt.rda")
  saveRDS(n_ext_final, "../../Figures_Full_species/when_where_extinction_all/n_ext_final_with_ttt.rda")
  saveRDS(ratio_final_group, "../../Figures_Full_species/when_where_extinction_all/ratio_final_group_with_ttt.rda")
  saveRDS(n_ext_final_group, "../../Figures_Full_species/when_where_extinction_all/n_ext_final_group_with_ttt.rda")
...
for (tttt in c(0, 1, 2)){
  for (da in c(0:1)){
    ...
    p
    ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/ratio_final_%d_ttt_%d.png", da, tttt), 
           width=9, height=4)
    ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/ratio_final_%d_ttt_%d.pdf", da, tttt),
           width=9, height=4)
    ...
    p
    ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/n_ext_final_%d_ttt_%d.png", da, tttt),
           width=9, height=4)
    ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/n_ext_final_%d_ttt_%d.pdf", da, tttt),
           width=9, height=4)
    ...
    for (g in c("Amphibians", "Birds", "Mammals", "Reptiles")){
      ...
      p
      ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/ratio_final_%d_%s_ttt_%d.png", da, g, tttt), 
             width=9, height=4)
      ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/ratio_final_%d_%s_ttt_%d.pdf", da, g, tttt),
             width=9, height=4)
      ...
      p
      ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/n_ext_final_%d_%s_ttt_%d.png", da, g, tttt),
             width=9, height=4)
      ggsave(p, filename=sprintf("../../Figures_Full_species/when_where_extinction_all/n_ext_final_%d_%s_ttt_%d.pdf", da, g, tttt),
             width=9, height=4)
    }
  }
}
