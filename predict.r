...
if (F){
  mask<-data.frame(rasterToPoints(raster("../../Raster/mask.tif")))
  env_layers<-list()
  for (i in c(1:nrow(layer_df))){
    ...
    layers_table<-mask
    layers_table[,c("PR", "TEMP_MAX", "TEMP_MIN")]<-extract(layers, mask[, c("x", "y")])
...
for (i in c(1:nrow(df_list))){
  ...
  for (target_folder in target_folders){
    ...
    for (j in c(1:nrow(layer_df))){
      ...
      print(paste(item$sp, layer_item$LABEL, j, nrow(layer_df), "/", i, nrow(df_list), target))
